...
def main():
    ...
    for _, r in tqdm(df.iterrows(), total=len(df), desc="Labeling triplets"):
        ...
        # 3-class trajectory:
        # 2 = monotonic up, 0 = monotonic down, 1 = mixed/flat
        if (d01 > thr) and (d12 > thr) and (d02 > thr):
            label = 2
        elif (d01 < -thr) and (d12 < -thr) and (d02 < -thr):
            label = 0
        else:
            label = 1
